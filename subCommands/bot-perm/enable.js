...
module.exports = {
    name: 'enable',
    parent: 'bot-perm',
    description: "Autoriser un utilisateur à utiliser le bot",
    guildOnly: true,
    args: true,
    ...
    execute(message,args,db) {
        ...
        db.query(`SELECT u.id_user, u.bot_permition FROM Users u WHERE ${UserKey} = ${UserValue}`, function(err,result, fields) {
            ...
            else
            {
                ...
                {
                    message.channel.send("Ce membre possède déja la permition d'utiliser le bot")
                }
                else
                {
                    db.query(`UPDATE Users SET bot_permition = 1 WHERE id_discord_user = ${member.id}`, function(err,result, fields) {
                        if(err)
                        {
                            func.log('err',err)
                        ...
                        {
                            message.channel.send(member.displayName + " a obtenu la permition d'utiliser le bot")
                        }
                    })
                }
            }
